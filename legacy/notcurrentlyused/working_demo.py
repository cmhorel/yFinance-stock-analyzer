"""
Working Demo - Recreating Original Functionality with New Architecture

This demonstrates the complete refactored system by:
1. Scraping NASDAQ-100 tickers from Wikipedia
2. Fetching sample stock data 
...
class WorkingStockAnalyzer:
    """Working stock analyzer using the new architecture."""
    
    def __init__(self, db_path: str = "data/working_demo.db"):
        self.settings = get_settings()
        self.logger = get_logger(__name__)
        self.repo = SqliteStockRepository(db_path=db_path)
        
        # Create plots directory
        self.plots_path = "plots/working_demo"
    ...
    async def sync_ticker_data(self, symbol: str) -> bool:
        ...
        try:
            ...
            # Fetch data from yfinance (simplified)
    ...
    def plot_all_tickers(self, df: pd.DataFrame):
        ...
        plt.title('Stock Prices Overview')
        plt.grid(True)
        plt.tight_layout()
...
async def main():
    ...
    logger.info("Starting working demo")
    
    print("🚀 Working Demo - Recreating Original Functionality")
    print("=" * 70)
    
    # Initialize analyzer
    analyzer = WorkingStockAnalyzer()
    ...
    # 2. Sync data (limit to first 10 for demo speed)
    demo_tickers = nasdaq_tickers[:10]  # Limit for demo
    print(f"\n2. Syncing Stock Data (Demo: {len(demo_tickers)} tickers):")
    
    for ticker in nasdaq_tickers:
        success = await analyzer.sync_ticker_data(ticker)
        if success:
            print(f"   ✅ {ticker}")
        else:
            print(f"   ❌ {ticker}")
    ...
    top_stocks = []
    if buy_candidates:
        top_stocks.extend([t[0] for t in buy_candidates[:2]])
    if sell_candidates:
        top_stocks.extend([t[0] for t in sell_candidates[:2]])
    ...
    print("  • Web scraping of NASDAQ-100 tickers")
    print("  • yFinance data fetching and storage")
    print("  • Clean architecture database operations")
